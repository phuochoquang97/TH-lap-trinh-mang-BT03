	...
	<form action="CheckShowNVServlet" method="post">
		...
		<table align="center" class="a">
			...
			<tr>
				...
				<td><a href="capnhatthongtinnv.jsp?manv=<%=nvlist.get(i).getMaNV()%>&hoten=<%=nvlist.get(i).getHoten()%>
				&gioitinh=<%=nvlist.get(i).getGioitinh()%>&phongban=<%=nvlist.get(i).getPhongban()%>">Cập nhật</a></td>
			<%
				}
				}
			%>
